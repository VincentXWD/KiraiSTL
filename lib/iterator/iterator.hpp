#ifndef KIRAI_LIB_ITERATOR_HPP
#define KIRAI_LIB_ITERATOR_HPP
...
template <typename Iterator>
typename iterator_traits<Iterator>::dfference_type*
...
void __advance(Iterator& i, Distance n, input_iterator_tag) {
  while (n-- > 0) {
    ++i;
  }
}
...
template <typename Iterator,typename Distance>
void __advance(Iterator& i, Distance n, bidirectional_iterator_tag) {
  if (n > 0) {
    while(n-- > 0) ++i;
  }
  else {
...
template <typename Iterator, typename Distance>
void __advance(Iterator& i, Distance n, random_access_iterator_tag) {
  i = i + n;
}
